...
Source adapters are intentionally **out-of-process**. Teams may write converters in any language as long as they produce the canonical JSON and pass validation. The repository ships reference adapters in `tools/AoD_generate_source_text_raw.py` (Age of Decadence) and `tools/HeadsWillRoll_generate_source_text_raw.py` (Heads Will Roll Reforged). Each walks its project’s raw assets, emits the canonical `source_text_raw.json`, companion per-file `source_text_raw_<file>.json` slices, a `metadata.enrichment.suggested.json`, and a `source_files_index.json`. Adapt these scripts—or roll your own—for other projects and check them into version control alongside your raw asset snapshots.

Adapters should report any unexpected translation shapes they encounter. The Heads Will Roll adapter, for example, tracks every handled line and supports `--strict-unhandled` to fail fast when new Ren’Py constructs appear, while warning-only runs still surface the offending lines for investigation.
